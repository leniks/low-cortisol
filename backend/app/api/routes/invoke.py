...
async def _stream_chat_answer(
    *,
    ...
    async for event in agent_service.run_stream(message, conversation_id=conversation_id):
...
async def invoke(body: InvokeRequest) -> InvokeResponse:
    ...
    final_parts: list[str] = []
    async for event in agent_service.run_stream(body.message, conversation_id=conversation_id):
        if event.get("type") == "final":
